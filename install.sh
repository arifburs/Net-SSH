...
userInputs(){
    ...
    echo -e "\n\n****** Welecome to installation of the NetSSH Panel ****** \n"
...
copyPanelRepo(){
   ...
   link=https://raw.githubusercontent.com/arifburs/Net-SSH/main/app.zip
...
configAppache(){
    ...
    echo '#NetSSH' > /var/www/netsshport
    sudo sed -i -e '$a\'$'\n''netsshport '$serverPort /var/www/netsshport
    wait
...
configDatabase(){
    ...
    if mysql -u root -e "USE netSSH" 2>/dev/null; then
        ...
        mysql -u root -e "DROP DATABASE netSSH;"
        echo "Old database 'netSSH' has been removed."
    else
        echo "Database 'netSSH' does not exist."
    fi

    sed -i "s/DB_DATABASE=netssh/DB_DATABASE=${dbName}/" /var/www/html/panel/.env
...
installationInfo(){
    ...
    clear
    echo -e "\n"
    bannerText=$(curl -s https://raw.githubusercontent.com/arifburs/Net-SSH/main/net.txt)
    printf "%s" "$bannerText"
    echo -e "\n"
...
nethogsLink=https://raw.githubusercontent.com/arifburs/Net-SSH/main/nethogs-json/install.sh
checkRoot
userInputs
updateShhConfig
installPackages
